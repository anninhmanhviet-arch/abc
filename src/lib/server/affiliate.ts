...
export const getMyAffiliate = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const friends = await sql<{
      user_id: string; email: string; display_name: string; created_at: unknown; deposited: string;
       ...
       where p.referred_by = ${context.userId} order by p.created_at desc limit 50`;
    const comms = await sql<{
      id: number; referee_id: string; email: string; base_amount: string; rate: string; amount: string; created_at: unknown;
    }>`select c.id, c.referee_id, u.email, c.base_amount, c.rate, c.amount, c.created_at
       from affiliate_commissions c join "user" u on u.id = c.referee_id
       where c.referrer_id = ${context.userId} order by c.created_at desc limit 40`;
    const me = await sql<{ referred_by: string | null }>`select referred_by from profiles where user_id = ${context.userId}`;
    return {
      code, rate, globalRate: global, enabled,
      friends: stats[0]?.friends ?? 0, earned: n(stats[0]?.earned),
      referred: Boolean(me[0]?.referred_by),
      rows: friends.map((f) => ({ email: maskEmail(f.email), name: f.display_name, createdAt: iso(f.created_at), deposited: n(f.deposited) })),
      commissions: comms.map((c) => ({ id: c.id, email: maskEmail(c.email), baseAmount: n(c.base_amount), rate: n(c.rate), amount: n(c.amount), createdAt: iso(c.created_at) })),
    };
  });

export const saveAffiliateSettings = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(z.object({ rate: z.string().min(1).max(8), enabled: z.boolean() }))
  .handler(async ({ context, data }) => {
    const sql = await getSql();
    ...
    await sql`insert into settings (key, value) values ('affiliate_rate', ${rate}) on conflict (key) do update set value = excluded.value`;
    await sql`insert into settings (key, value) values ('affiliate_enabled', ${enabled}) on conflict (key) do update set value = excluded.value`;
...
export const getAdminAffiliate = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const q = (data.q ?? "").trim();
    const like = q ? `%${q}%` : null;
    const people = await sql<{ user_id: string; email: string; referral_code: string | null; affiliate_rate: string | null }>`
      select p.user_id, u.email, p.referral_code, p.affiliate_rate
      from profiles p join "user" u on u.id = p.user_id
      where coalesce(p.role, 'user') <> 'admin'
        and (${like}::text is null or u.email ilike ${like} or coalesce(p.referral_code, '') ilike ${like} or p.display_name ilike ${like})
      order by p.created_at desc limit 100`;
    const friendRows = await sql<{ referrer_id: string; friends: number }>`
      select referred_by as referrer_id, count(*)::int as friends from profiles where referred_by is not null group by referred_by`;
    const earnRows = await sql<{ referrer_id: string; earned: string }>`
      select referrer_id, coalesce(sum(amount), 0)::text as earned from affiliate_commissions group by referrer_id`;
    ...
    const recent = await sql<{
      id: number; referrer_email: string; referee_email: string; base_amount: string; rate: string; amount: string; created_at: unknown;
    }>`select c.id, r.email as referrer_email, f.email as referee_email, c.base_amount, c.rate, c.amount, c.created_at
       from affiliate_commissions c join "user" r on r.id = c.referrer_id join "user" f on f.id = c.referee_id
       order by c.created_at desc limit 40`;
    return {
      rate: global, enabled, paid: n(tot[0]?.c),
      leaders: people.map((l) => {
        const custom = l.affiliate_rate == null || l.affiliate_rate === "" ? null : n(l.affiliate_rate);
        return {
          userId: l.user_id, email: l.email, code: l.referral_code ?? "",
          friends: friendsMap.get(l.user_id) ?? 0, earned: earnMap.get(l.user_id) ?? 0,
          customRate: custom, effectiveRate: custom ?? global,
        };
      }),
      recent: recent.map((r) => ({
        id: r.id, referrer: r.referrer_email, referee: r.referee_email,
        baseAmount: n(r.base_amount), rate: n(r.rate), amount: n(r.amount), createdAt: iso(r.created_at),
      })),
    };
  });
