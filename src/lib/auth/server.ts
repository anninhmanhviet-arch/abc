...
const extraTrusted = [
  ...
  .map((s) => s.trim())
  .filter(Boolean)
  .map((s) => (s.startsWith("http") ? stripSlash(s) : `https://${s.replace(/\/+$/, "")}`));
...
const trustedOrigins: string[] = [
  ...
  ...LOCAL_DEV_ORIGINS,
  ...extraTrusted.flatMap(originVariants),
  "https://vertex.id.vn",
  "https://www.vertex.id.vn",
  "https://vertex-abcbode.vercel.app",
];
